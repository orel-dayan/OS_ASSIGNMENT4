...
## Description:
The project is demonstration of a server that uses the Reactor design pattern to handle events generated by multiple clients.
 This means that it doesn't waste CPU cycles polling clients that have no new messages. 
 Instead, it uses the OS's ability to "react" to events such as new client connections or incoming messages. 
 The functionality of the reactor provided by shared library that linked to the server, the libary is not thread safe.
